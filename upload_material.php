...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if ($_FILES["notes_files"]["error"][$i] === 0) {
                ...
                if (move_uploaded_file($tmpName, $file_path)) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        echo "❌ Notes Insert Error: " . $stmt->error . "<br>";
                    }
                } else {
                    echo "❌ Failed to move uploaded notes file: " . $_FILES["notes_files"]["name"][$i] . "<br>";
                }
            } else {
                echo "❌ Upload error for note file: " . $_FILES["notes_files"]["name"][$i] . " | Error Code: " . $_FILES["notes_files"]["error"][$i] . "<br>";
            }
        }
    }
            ...
            if ($_FILES["book_files"]["error"][$i] === 0) {
                ...
                if (move_uploaded_file($tmpName, $file_path)) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        echo "❌ Book Insert Error: " . $stmt->error . "<br>";
                    }
                } else {
                    echo "❌ Failed to move uploaded book file: " . $_FILES["book_files"]["name"][$i] . "<br>";
                }
            } else {
                echo "❌ Upload error for book file: " . $_FILES["book_files"]["name"][$i] . " | Error Code: " . $_FILES["book_files"]["error"][$i] . "<br>";
            }
        }
    }
    ...
    if ($success > 0) {
        echo "✅ $success item(s) uploaded successfully.";
    } else {
        echo "❌ No files or links were uploaded. Please check for errors above.";
    }
}
?>